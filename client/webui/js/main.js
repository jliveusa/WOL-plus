...
const App = {
    ...
    bindEvents() {
        ...
        // 网卡选择变化 - 自动填充该网卡的 MAC 地址
        document.getElementById('networkInterface').addEventListener('change', () => {
            Network.handleSelectionChange();
        });
    },
     ...
     * 处理应用网卡/MAC 地址设置
     */
    async handleApplyNetwork() {
        const result = await Network.apply();
    ...
    async handleSaveSettings() {
        ...
        if (result.success && !result.needRelogin && !result.unchanged) {
            await Config.load();
        }
