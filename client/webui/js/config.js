...
const Config = {
    ...
    readSettingsFromDOM() {
        ...
        const payload = {
            ...
            port: document.getElementById('managementPort').value.trim()
        };
    ...
    populateDOM(data) {
        ...
        Network.applyCurrentConfig(data);
        ...
        document.getElementById('managementPort').value = data.port || '2025';
        document.getElementById('newPassword').value = '';
        this.settingsBaseline = {
            username: document.getElementById('usernameInput').value,
            extra_data: ExtraInput.getValue(),
            shutdown_delay: document.getElementById('shutdownTime').value,
            port: document.getElementById('managementPort').value
        };
    },
    ...
    async saveSettings() {
        ...
        if (!/^\d+$/.test(settings.port) || portNumber < 1 || portNumber > 65535) {
            return { success: false, message: I18n.t('error.port') };
        }

        const payload = {};
        const settingsFields = ['username', 'extra_data', 'shutdown_delay', 'port'];
        ...
        const portChanged = Object.prototype.hasOwnProperty.call(payload, 'port');
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (portChanged) {
                    return {
                        success: true,
                        message: I18n.t('success.portSaved', { port: settings.port })
                    };
                }
